export const formatPhysicsFormula = (text: string | undefined) => {
  ...
  formatted = formatted.replace(nuclearRegex, (_match, p1, p2, p3) => {
  ...
  formatted = formatted.replace(powerRegex, (_match, p1) => {
    return `<sup>${p1}</sup>`;
  });

  formatted = formatted.replace(subscriptRegex, (_match, p1) => {
